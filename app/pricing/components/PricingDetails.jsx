        ...
        {pricingData.map((section, index) => (
          ...
          >
            ...
            <div className="mb-4 md:mb-0 md:w-[40%]">
              <h3 className="text-white text-xs sm:text-sm md:text-base font-medium tracking-wide uppercase whitespace-pre-line">
                {section.category}
              </h3>
            </div>

            {/* Right - Pricing Items */}
            <div className="md:w-[50%] flex flex-col gap-3 md:gap-4">
              {section.items.map((item, itemIndex) => (
                <div key={itemIndex} className="flex items-start gap-2 sm:gap-3">
                  <span className="text-text-dim mt-0.5 md:mt-1">•</span>
                  <div>
                    {item.label && (
                      <span className="text-text-dim text-xs sm:text-sm md:text-base block mb-1">
                        {item.label}
                      </span>
                    )}
