  ...
  return (
    <section className="section-solid w-full py-12 md:py-20 px-4 sm:px-8 md:px-16">
      <div className="max-w-4xl mx-auto flex flex-col items-center text-center">
        ...
        {/* Main Description */}
        <p className="text-text-dim text-base sm:text-xl md:text-2xl lg:text-[28px] leading-relaxed mb-4 md:mb-6">
          Compell is a digital product and software studio that builds, scales, and sells apps for modern businesses.
        </p>

        <p className="text-text-dim text-base sm:text-xl md:text-2xl lg:text-[28px] leading-relaxed mb-10 md:mb-16">
        ...
        {/* Tags */}
        <div className="flex flex-col items-center gap-2 sm:gap-3 mb-8 md:mb-10">
          {tags.map((row, rowIndex) => (
            <div key={rowIndex} className="flex flex-wrap justify-center gap-2 sm:gap-3">
              {row.map((tag, tagIndex) => (
                <div
                  key={tagIndex}
                  className="px-3 py-1.5 sm:px-4 sm:py-2 bg-[#070101] text-text-dim text-xs sm:text-sm tracking-wide"
                >
                  {tag}
                </div>
              ))}
        ...
        {/* CTA Button */}
        <GetInTouchButton className="px-4 sm:px-6" />
      </div>
    </section>
  )
}
